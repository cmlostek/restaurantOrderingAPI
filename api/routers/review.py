...
router = APIRouter(
    prefix="/users/{user_id}/reviews", tags=["reviews"]
)

@router.get("/", response_model=List[Review])
def get_all_reviews(user_id: int, db: Session = Depends(get_db)):
    reviews = get_all_reviews_by_user(db, user_id)
    if not reviews:
        raise HTTPException(status_code=404, detail="No reviews found for your user ID")
    return reviews
...
def create_review_entry(request: Review, user_id: int, db: Session = Depends(get_db)):
    return create_review(request, user_id, db)
